...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Google Sheets Senne videos response received');
    ...
    const videos = (data.values || [])
      ...
      .map((row: string[]) => ({
        ...
        revenue: parseAmount(row[3] || '0'), // Column D - Revenue
        tax: parseAmount(row[4] || '0'), // Column E - Tax
        revenueAfterTax: parseAmount(row[5] || '0'), // Column F - Revenue After Tax
        publishDate: row[7] || '', // Column H
        minutesWatched: parseViews(row[8] || '0'), // Column I
        avgDuration: row[9] || '', // Column J
        likes: parseViews(row[10] || '0'), // Column K
        shares: parseViews(row[11] || '0'), // Column L
        subsGained: parseViews(row[12] || '0'), // Column M
        thumbnailUrl: row[13] || '', // Column N
      }));

    console.log(`Parsed ${videos.length} Senne Only videos`);
    ...
    console.error('Error in get-senne-videos function:', error);
