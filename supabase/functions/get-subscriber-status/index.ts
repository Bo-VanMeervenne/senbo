...
serve(async (req) => {
  ...
  try {
    ...
    // Skip header row
    const subscriberData = rows.slice(1).map((row: string[]) => ({
      status: row[0] || '',
      views: parseNumber(row[1]),
      minutesWatched: parseNumber(row[2]),
    })).filter((item: any) => item.status);

    console.log(`Processed ${subscriberData.length} subscriber statuses`);
